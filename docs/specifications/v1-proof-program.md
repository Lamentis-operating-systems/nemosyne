...
| `ALG-EXP-06` | [Representative medoid](predictive-attention-and-expectation.md#representative-medoid) | Every nonempty positive-support canonical outcome group, optional registered dissimilarity, and any independently authored lexical surface to exactly one stored representative; finite limits, medoid when distance is available, and total fallback otherwise. Surface authoring is not an input to representative identity, exact-sidecar ownership, tie-break, or ordering | Deterministic representative without truth or metric claims; test unavailable and nontransitive distances, every tie component, and authored-surface-present/absent pairs with identical representative and sidecar results | `EXP-02`, `PERF-01` | F10, F15; G4 |
