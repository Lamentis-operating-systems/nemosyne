...
| `PLAN-01` | **Construct focus candidates.** Own `deriveRequestPropositions(Q, Lambda_A, K)`, exact \(B_Q=\pi_Q(\Lambda_A)\), its ephemeral `RequestPropositionSet`, five-field receipt copied solely from \(\Lambda_A\), pure authority-lowering source mapping, the complete `FocusCandidateSet`, and request-local/memory-supported compatible-proposition consolidation over one frozen activated input; receive no principal, policy object, authorization view/service, or direct authorization projection and perform no authorization, expectation derivation, final budget selection, shared-set construction, or persistent write | `CORE-02`, `EVAL-02`, `SIT-01` | Q schema/no-auth dependency, mismatched request/situation/configuration, prompt/situation/metadata locator/content binding, five-field receipt lineage, all twelve typed reasons, duplicate, conflict, authority-ceiling, provenance, bounded-candidate, and empty-memory situation-only focus tests | G4 entrance and G7 |
...
| `SIT-01` | **Construct the numerical situation.** Implement \(Q=\operatorname{encode}(P,S,\Xi;K)\) over the validated prompt, ordered zero-to-three situation statements, caller-supplied contextual time, optional declared location, explicit metadata, and pinned encoder/configuration inputs; emit \(B_Q\), validated locators, and source-buffer content identities; do not receive trusted authorization time or discover ambient state or infer authorization | `CORE-02`, `ENC-01`, `TGT-01` | Field-ablation, missing/unknown, language, contextual-time/location, situation order/count, schema, encoder-incompatibility, locator/content-identity, deterministic replay, trusted-time/private-authorization noninterference, and no-discovery tests | G6 and G7 entrance |
...
| `API-01` | **Implement the callable compiler API.** Add public externally constructible `InstallationLocator`, `PromptOriginPresentation`, `CompileCallClaims`, request, cancellation, open, and `compile(&CompileCallClaims, &CompileRequest, &CancellationToken)` boundaries; locator construction from only closed untrusted schema/scope/identity selectors; origin construction from only one versioned route tag and bounded opaque bytes; closed intrinsic constructor errors; compiler-owned installation/registry/bootstrap-trust/handle resolution; the sole compiler-owned `LocalPlatformAuthenticator`; a fresh crate-private `InvocationContext` and private context-taking core; authenticated installed-configuration resolution; compatibility and stage errors; cancellation; exact in-memory framing; stable typed sources; and one complete result or error. Expose no caller-supplied path, manifest, trust root, registry, credential, platform handle, terminal transport, memory management, trusted-context constructor, or private core | `COMP-01`, `EXP-03`, `PLAN-02`, `REN-06`, `MEM-03`, `SEC-00`, `TGT-01` | External downstream-crate construction/open/one-call cases; compile-fail privacy for locator/presentation/claims/context and all trust/handle inputs; every closed constructor error; absent-locator open failure; prompt-byte properties; all-field locator/origin/claim/context forgery; no environment/current-directory/path fallback; in-process/helper topology; construction versus open/compatibility separation; cancellation before/during authentication and core; forbidden capability; total request-proposition reason mapping; and typed-source tests | G8 entrance |
| `CLI-01` | **Implement the CLI adapter.** Add argument parsing, request bytes, public `InstallationLocator` construction from the closed V1 current-user scope and package-defined installation identity, public `PromptOriginPresentation` construction from the registered versioned CLI route and bounded opaque bytes, bounded `CompileCallClaims`, requested installed-configuration-identity transport, public open/compile invocation, buffered stdout attempt, diagnostic separation, stable typed exit mapping, and partial-transport failure behavior; accept no installation path, trust root, registry, credential, or platform handle; construct no `InvocationContext`; authenticate no presentation; perform no installation/trust/configuration resolution; and duplicate no compiler logic | `API-01` | CLI/library golden parity, arbitrary-byte ingress, closed-constructor and open-error mapping, locator/origin/claim/context-forgery rejection, no path/trust/handle option, claims/request-construction-versus-compatibility mapping, complete request-proposition exit mapping, broken-pipe, partial-write, and discard-rule tests | G8 |
...
| `PLAN-01` | Authority-free validated `Q` with \(B_Q\)/locators/content identities, pinned `K`, and complete shared set carrying sole \(\Lambda_A\) → exact join, ephemeral `RequestPropositionSet`, and complete `FocusCandidateSet`; no direct authorization input/call, expectation truth, shared-set construction, final budget selection, or persistence | `nemosyne-core` focus module | Implementation / P0 / L-medium / planning / Normal / M3 | Preserve exact source locators, receipt lineage from \(\Lambda_A\), exclusions, allowed-use ceilings, typed failures, and authority; situation-only focus must survive an empty memory set; revert request-local module independently |
| `PLAN-02` | Focus and expectation sets → canonical bounded plan; no lexical generation or approximation | `nemosyne-core` planning module | Implementation / P0 / L-medium / planning / Normal / M3 | No action or answer fields; exhaustive oracle retained as rollback and reference |
...
| `SIT-01` | \(P,S,\Xi\) and pinned \(K\)/encoders → deterministic `Q`, three-field \(B_Q\), locators, and content identities; no trusted time, discovery, or authority inference | compiler/core situation module | Implementation / P1 / L-medium / situation / Normal / M3 | Contextual time, location, and metadata remain untrusted request evidence; principal, policy, authorization-view, and `t_auth` perturbations cannot affect `Q`; reject incompatible schema and roll back encoder/config identity |
...
| `API-01` | Closed untrusted installation selectors and origin bytes plus qualified components → public locator/presentation/claims constructors, compiler-owned installation/bootstrap-trust/registry/handle resolution, sole local authenticator, fresh private context, and in-memory callable compile API with split construction, open, compatibility, and stage errors; no caller path/trust/handle input, terminal transport, memory management, or downstream model call | new compiler crate and private installation-resolution/platform-authentication modules | Integration / P1 / L-medium / integration / Normal / M5 | An external crate can construct the complete public call path but cannot name or construct context/private core or supply paths, roots, registries, credentials, or handles; locator/presentation/claims cannot raise authority; open/trust/configuration resolution, cancellation, exact bytes, and total typed error ownership remain compiler-owned; rollback compiler/config atomically |
| `CLI-01` | Callable API → public untrusted locator, origin-presentation, request, and installed-identity claim transport plus delivery adapter; no path/trust/handle input, `InvocationContext` constructor, presentation authentication, installation/trust/configuration resolution, compiler logic, or management command | new CLI crate | Integration / P1 / M-high / integration / Normal / M5 | Untrusted selectors, arguments, and presentations are bounded; constructor/open/compatibility/stage errors map by type; diagnostics are separated; no migration; rollback adapter independently when API compatible |
...
| `PLAN-01` | Focus owner / derives request-supported propositions after an exact shared-lineage join and produces complete evidence-bound focus candidates without reauthorizing | `V1-R01`, `V1-R06` through `V1-R08`, `V1-R15`, `V1-R25`; `ARCH`, `MEM`, `PLAN` | `D14`, `D15` | `RequestPropositionSet`, focus API, \(B_Q=\pi_Q(\Lambda_A)\), source-receipt binding solely from \(\Lambda_A\), authority-lowering mapping, and consolidation definitions; no authorization API, expectation truth, or persistence migration | Q has no policy/auth-view field; static no-authorization dependency; source-locator/content-identity, join mismatch, five-field receipt, all twelve error reasons, duplicate, conflict, authority, bounded-candidate, and empty-memory situation-only tests |
...
| `SIT-01` | Situation owner / creates one explicit request-local numerical state and validated request-source bindings | `V1-R01`, `V1-R10`, `V1-R11`, `V1-R25`; `ARCH`, `MEM`, `PROOF` | New ADR only if encoder composition is selected | Situation API, deterministic transform, and exact source-byte locators in `X_Q`; no focus consolidation or persistent migration | Missing/unknown, source-locator, ablation, order, time, language, and no-discovery tests |
...
| `API-01` | Compiler API, installation resolver, and platform-authenticator owner / exposes the single reusable local call while keeping trust acquisition private | `V1-R01` through `V1-R10`, `V1-R12`, `V1-R13`, `V1-R15` through `V1-R25`; `PC`, `ARCH`, `PROOF` | New ADR for stabilized locator/presentation/claims/API/error/context/topology compatibility | Public `InstallationLocator`, `PromptOriginPresentation`, `CompileCallClaims`, open and compile API; compiler-owned bootstrap trust, registry and platform-handle resolution; `LocalPlatformAuthenticator`; crate-private context/core; total typed error map; and Rustdoc; composition only, no new mathematics or persistent migration | External-crate complete-path callability/privacy; locator/presentation/claims/context forgery; no-fallback; topology; boundary; byte; cancellation before/during authentication/core; capability; retryability; and typed-source tests |
| `CLI-01` | CLI transport owner / makes the public API callable without supplying or authenticating trust or duplicating semantics | `V1-R01` through `V1-R03`, `V1-R08` through `V1-R10`, `V1-R25`; `PC`, `ARCH` | New ADR only for stable CLI compatibility | CLI public locator/presentation/request/bounded-claims/installed-identity transport contract and typed exit map; no path/root/registry/credential/handle input, context construction, presentation authentication, installation/trust/configuration resolution, math, or storage migration | Golden library parity, closed-constructor/open mapping, locator/presentation/claims/context forgery, complete error-class mapping, broken transport, exit and discard tests |
...
| `IF-SITUATION` | `SIT-01` | `RET-01`, `SIG-01`, `PLAN-01`, diagnostics | \(Q=\operatorname{encode}(P,S,\Xi;K)\), exact \(B_Q=(request,situation,configuration)\), and validated source-byte locators/content identities in `X_Q` from prompt, ordered zero-to-three situations, untrusted contextual time, optional location, explicit metadata, and pinned encoder/configuration inputs; no trusted time, principal, policy, or authorization fields | Identical \(P,S,\Xi,K\) gives identical facets, \(B_Q\), locators, and content identities; changing only principal, `t_auth`, policy, or authorization view cannot change them; malformed/mismatched locators and injected trusted fields fail; static-dependency audit proves no authorization service, ambient discovery, authority inference, raw-text reread by focus, or focus consolidation |
...
| `IF-SHARED-SET` | `COMP-01` | `PLAN-01`, `EXP-02`, `API-01` | Complete eligible activated-memory set carrying the sole complete \(\Lambda_A\), including policy-revision and authorization-view identities, plus authority, conflicts, limits, and canonical order | Deterministic reconstruction, exact \(B_Q=\pi_Q(\Lambda_A)\) join, and projection noninterference; no proposition consolidation or repeated authorization |
...
| `IF-COMPILE-API` | `API-01` | External Rust callers, `CLI-01`, `OBS-01`, `SEC-01`, `PERF-01`, `SYS-01` | `InstallationLocator::new` accepts only closed versioned untrusted installation schema/scope/identity selectors; `PromptOriginPresentation::new` accepts only a versioned declared route tag and bounded opaque bytes; `Compiler::open(&InstallationLocator)` resolves authenticated installation state, bootstrap trust, registries, and platform handles itself; and `compile(&CompileCallClaims, &CompileRequest, &CancellationToken)` is the single local read-only call. Claims contain only the private presentation and optional installed-configuration/narrower-disclosure requests. The compiler-owned authenticator combines the presentation with compiler-owned handles, code/channel identity, trusted clock, and authenticated registries to derive a fresh crate-private context before the private core. Closed locator/presentation/claims/request errors remain separate from open, compatibility, and stage errors; cancellation returns one typed error and success returns complete bytes | External downstream-crate construction/open/callability and compile-fail privacy; constructor exhaustiveness; absent-locator open failure; all-field locator/presentation/claims/context forgery; proof that no caller path, root, registry, credential, or handle is accepted; no environment/current-directory/path fallback; in-process/helper topology; complete request-proposition reason/exit/retryability mapping; byte, capability, cancellation-before/during-authentication/core tests; compiler/configuration rolls back atomically |
| `IF-CLI` | `CLI-01` | Humans, agents, `SEC-01`, `PERF-01`, `SYS-01` | Terminal request bytes; public locator construction from the closed V1 current-user scope and package-defined installation identity; public origin-presentation construction from the registered versioned route and bounded opaque bytes; bounded call claims and requested installed-identity transport; public open/compile calls; stdout framing; stderr diagnostics; typed exit map; and discard-on-failure rule | Golden library parity, closed-constructor/open mapping, locator/presentation/claim/context forgery, construction-versus-open/compatibility and complete stage-error mapping, and broken-pipe tests; no caller path/root/registry/credential/handle option, presentation authentication, trusted-context construction, installation/trust/configuration resolution, compiler, or management semantics |
...
→ MEM-01/02/03
  ├─ request-focus lane: ENC-01 → SIT-01 → PLAN-01 → PLAN-02
  │  → REN-01/02 → VAL-01 → REN-04 → optional REN-03/05 → REN-06 and G4
  ├─ activation lane: ENC-01 → SIT-01 → RET-01 → SIG-01 → ACT-00
  │  → ACT-01 → COMP-01
...
| `CP-SEMANTIC` | `{EVD-02, BND-01} → CORE-01 → EVAL-01 → EVAL-02 → EXP-01 → EXP-02 → EXP-03`, joined with `{CORE-02, EVAL-02, SIT-01} → PLAN-01 → PLAN-02` | Defines typed predictive and request-bound planning truth consumed by every renderer and end-to-end evaluation | Reject invalid domain, request-source binding, or algorithm; retain exhaustive references and failed receipts |
...
| `CP-MEMORY` | `{CORE-02, SEC-00, TGT-01} → MEM-01 → MEM-02 → ENC-01 → SIT-01`, then `RET-01 → SIG-01 → ACT-00 → ACT-01 → COMP-01` and the joined `PLAN-01` request-focus branch | Produces both the validated request-local situation/source bindings and the complete authorized activated set | Exact-scan and prior-kernel references support rollback; failed authorization, source binding, or recovery stops |
...
| `M3` | Establish authorized memory-to-activation and close the request-bound plan; entrance is M2 | `MEM-01..05`, `ENC-01`, `SIT-01`, `RET-01`, `SIG-01`, `ACT-00`, `ACT-01`, `COMP-01`, `PLAN-01`, `PLAN-02` | G5 through G7, planning entrance, and recovery/privacy readiness; permits renderer work | Data loss, authority leak, request-source mismatch, unrecoverable migration, missing activation evidence, or unclosed plan |
...
| `W11` | `PLAN-01` ∥ `RET-01` | Request-bound focus candidates and authorized retrieval |
| `W12` | `PLAN-02` ∥ `SIG-01` | Closed plan and typed signals |
| `W13` | `REN-01` ∥ `ACT-00` | Deterministic lexical baseline and activation candidate |
| `W14` | `REN-02` ∥ `ACT-01` | Renderer corpus and adopted activation |
| `W15` | `VAL-01` ∥ `COMP-01` ∥ optional `REN-03` | Frozen validator, shared set, and optional model artifacts |
| `W16` | `REN-04` | Qualified deterministic renderer |
| `W17` | optional `REN-05` | Optional model disposition, or explicit waived cohort |
| `W18` | `REN-06` | Authenticated renderer-validator deployment |
| `W19` | `API-01` | Reusable one-call compiler boundary |
| `W20` | `CLI-01` ∥ `OBS-01` | Transport and authorized diagnostics |
| `W21` | `SEC-01` ∥ `PERF-01` | Independent security and operational receipts |
| `W22` | `SYS-01` | G8 vertical-slice receipt |
| `W23` | `REL-01` | Frozen content-identified candidate |
| `W24` | `RCV-01` | Exact-package verification receipt |
| `W25` | `E2E-01` | Permanent G9 receipt |
| `W26` | `REL-02` | Signed ship authorization or stop receipt |
| `W27` | `REL-03` | Verified shipment or publication rollback receipt closing G10 |
...
| CLI and diagnostics | `API-01` is merged; only its compiler-owned installation resolver and platform authenticator acquire trust and construct the crate-private `InvocationContext`; both consumers call the public locator/presentation/claims/request boundary and neither reimplements compiler semantics | Public acquisition/errors, installation or trust resolution, trusted-context construction, installed-configuration resolution, result bytes, redaction, or authority changes serialize through `API-01`/`SEC-00` |
...
| --- | --- | --- | --- |
| `V1-R01` | `TGT-01`, `CORE-02`, `SIT-01`, `PLAN-01`, `API-01`, `CLI-01` | Intrinsic locator/presentation/claims/request versus open/installed-compatibility errors, external-crate complete-path callability, deterministic \(P,S,\Xi,K\)-to-\(Q\) evidence, source locators/content identities, exact \(B_Q=\pi_Q(\Lambda_A)\), five-field request-proposition receipts, all twelve reason mappings, empty-memory situation-only focus, origin, zero-to-three count/order, contextual-time/location/metadata presence, language, trusted-time noninterference, and forged-time authority-isolation cases | G8 |
...
| `V1-R10` | `SEC-00`, `API-01`, `CLI-01`, `SEC-01`, `RCV-01` | Private compiler-owned installation/trust/handle resolution and context construction; public locator/presentation/claims/context compile-fail privacy; all-field library/CLI locator, origin-presentation, and claim forgery rejection; no environment/current-directory/caller-path trust fallback; in-process versus authenticated-peer runtime topology; no direct focus-authorization edge/call; prohibited capability; and dependency-graph audits | G8/G10 |
...
| --- | --- | --- | --- | --- |
| `V1-R01` | `PC`, `ARCH` input | `IF-DOMAIN-REQUEST`, `IF-SITUATION`, `IF-FOCUS-CANDIDATES`, `IF-COMPILE-API`, `IF-CLI` | Intrinsic malformed locator/presentation/claims/request, valid-but-unresolvable locator, valid-but-incompatible request, external-crate complete public call, deterministic Q-domain and trusted-time/private-authorization noninterference, source-locator/content-identity/receipt mismatch, exact request/situation/configuration join, total request-proposition reason map, empty-memory situation-only focus, language/principal, and unknown/trusted-field fixtures | G8 boundary receipt |
...
| `V1-R10` | `PC`, `ARCH` process capability | `IF-SECURITY-ARCH`, `IF-COMPILE-API`, `IF-CLI` | Library and CLI all-field locator/presentation/claim/context forgery, public trust-root/registry/credential/handle and private-core compile-fail privacy, no environment/current-directory/caller-path fallback, forbidden management/network/downstream-model construction, sole installation-resolver/authenticator ownership, Q/focus no-authorization-edge static audit, and in-process/authenticated-peer topology audit | G8/G10 |
...
| `OD-03` / Open | Stable externally constructible `InstallationLocator`, `PromptOriginPresentation`, `CompileCallClaims`, request/result, open, and compile contracts; closed locator/presentation/claims/request constructor errors versus open/configuration compatibility and stage errors; total request-proposition reason mapping; retryability; cancellation/resource behavior; CLI transport/usage/exit mapping remains adapter-owned | `API-01` with `BND-01`, `SEC-00`, and `TGT-01`; `CLI-01` only constructs the accepted untrusted public inputs and maps the accepted typed API | Accepted API ADR plus external-crate complete-path callability/privacy, constructor/open/compatibility separation, complete typed-source/exit/retryability, and cancellation fixtures; blocks `API-01`, then `CLI-01` |
| `OD-04` / Open | OS identity, installation principal, bootstrap trust and authenticated installation-registry resolution, caller capability, origin attestation, compiler-owned handles/channel binding/trusted clock, in-process versus helper/service isolation, untrusted locator/claims/metadata treatment, and the `API-01` authenticator that combines public presentation bytes with compiler-owned trust sources and solely constructs crate-private `InvocationContext`; callers can supply no path, manifest, trust root, registry, credential, or platform handle | `SEC-00` before `CORE-02` and `MEM-01`; `API-01` implements, `CLI-01` only transports bounded untrusted public selectors, presentations, claims, and requests | Security/topology ADR, all-field locator/presentation/claims forgery and privacy misuse cases, compile-fail trust/handle privacy, and sole-owner/no-fallback receipt; blocks domain/security consumers |
