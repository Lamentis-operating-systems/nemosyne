...
| ID | Concise title, objective, scope, and non-scope | Depends on | Acceptance and exit evidence | Contributes to |
| --- | --- | --- | --- | --- |
| `EXP-01` | **Implement predictive domain contracts.** Add transition, condition, horizon, outcome, status, alternative-family, query, and typed `TransitionReliability { schema, state }` records; `ReliabilityState` is exactly `Derived { value, derivation, calibration_domain, migration }`, `Missing`, `Unknown`, or `Inapplicable`, and any `ReliabilityMigrationRef` binds migration ID plus exact source record revision/schema/domain/state digest; no prediction algorithm or compile-time auto-migration | `CORE-02`, `EVAL-02` | Constructor, state-identity, derived `0`/`1`, unavailable-state, malformed-identity, schema/domain compatibility, authenticated migration, stale/missing/mismatched migration, contradiction, exclusivity, and lineage tests | G3 and G7 |
...
| `PLAN-01` | **Construct focus candidates.** Own `deriveRequestPropositions(Q, Lambda_A, K)`, its ephemeral `RequestPropositionSet`, the complete `FocusCandidateSet`, and all request-local and memory-supported compatible-proposition consolidation over one frozen activated input; no expectation evidence, final budget selection, shared-set construction, or persistent memory | `CORE-02`, `EVAL-02`, `SIT-01` | Prompt/situation/metadata source-binding, receipt-lineage, duplicate, conflict, authority-ceiling, provenance, bounded-candidate, and empty-memory situation-only focus tests | G4 entrance and G7 |
...
| `API-01` | **Implement the callable compiler API.** Add `nemosyne-compiler` open and compile boundaries, the sole platform-authenticated `InvocationContext` adapter, authenticated installed-configuration resolution, intrinsic `CompileRequestError` versus compiler/configuration compatibility errors, cancellation, exact in-memory framing, stable typed errors, and one complete result or error; no terminal transport or memory management | `COMP-01`, `EXP-03`, `PLAN-02`, `REN-06`, `MEM-03`, `SEC-00`, `TGT-01` | One-call golden cases, prompt-byte properties, trusted-context forgery rejection, request-construction/compatibility error separation, cancellation, forbidden-capability, and typed-error tests | G8 entrance |
| `CLI-01` | **Implement the CLI adapter.** Add argument parsing, request bytes, prompt-origin material, and requested installed-configuration-identity transport, API invocation, buffered stdout attempt, diagnostic separation, stable typed exit mapping, and partial-transport failure behavior; construct no `InvocationContext`, perform no trust/configuration resolution, and duplicate no compiler logic | `API-01` | CLI golden cases, arbitrary-byte ingress, context-forgery rejection, construction-versus-compatibility exit mapping, broken-pipe, partial-write, and discard-rule tests | G8 |
...
| `PLAN-01` | Validated `Q`, matching `Lambda_A` projection, pinned `K`, and shared activated set → ephemeral `RequestPropositionSet` and complete `FocusCandidateSet`; no expectation truth, shared-set construction, final budget selection, or persistence | `nemosyne-core` focus module | Implementation / P0 / L-medium / planning / Normal / M3 | Preserve exact source locators, exclusions, allowed-use ceilings, and authority; situation-only focus must survive an empty memory set; revert request-local module independently |
...
| `API-01` | Qualified components plus platform trust inputs → sole trusted invocation adapter and in-memory callable compile API with split request-construction/configuration compatibility errors; no terminal transport, memory management, or downstream model call | new compiler crate and platform-authentication adapter module | Integration / P1 / L-medium / integration / Normal / M5 | CLI cannot forge context; trust/configuration resolution, cancellation, exact bytes, and typed error ownership remain compiler-owned; rollback compiler/config atomically |
| `CLI-01` | Callable API → terminal request/origin/identity transport and delivery adapter; no `InvocationContext` constructor, trust/configuration resolution, compiler logic, or management command | new CLI crate | Integration / P1 / M-high / integration / Normal / M5 | Untrusted arguments bounded, construction and compatibility errors map by type, diagnostics separated, no migration; rollback adapter independently when API compatible |
...
| `PLAN-01` | Focus owner / derives authenticated request propositions and produces complete evidence-bound focus candidates | `V1-R01`, `V1-R06` through `V1-R08`, `V1-R15`, `V1-R25`; `ARCH`, `MEM`, `PLAN` | `D14`, `D15` | `RequestPropositionSet`, focus API, source-receipt binding, and consolidation definitions; no expectation truth or persistence migration | Source-locator, five-field receipt, duplicate, conflict, authority, bounded-candidate, and empty-memory situation-only tests |
...
| `API-01` | Compiler API and platform-invocation-adapter owner / exposes the single reusable local call | `V1-R01` through `V1-R10`, `V1-R12`, `V1-R13`, `V1-R15` through `V1-R25`; `PC`, `ARCH`, `PROOF` | New ADR for stabilized API/error/context compatibility | Public Rust API, sealed trusted-context construction, and Rustdoc; composition only, no new mathematics or persistent migration | Intrinsic-request versus compatibility errors, context forgery, boundary, byte, cancellation, capability, and typed-source tests |
| `CLI-01` | CLI transport owner / makes the API callable without constructing trust or duplicating semantics | `V1-R01` through `V1-R03`, `V1-R08` through `V1-R10`, `V1-R25`; `PC`, `ARCH` | New ADR only for stable CLI compatibility | CLI request/origin/identity transport contract and typed exit map; no context construction, trust/configuration resolution, math, or storage migration | Golden input/output, context-forgery, error-class mapping, broken transport, exit and discard tests |
...
| `IF-DOMAIN-REQUEST` | `CORE-02` | `SIT-01`, memory authorization, `API-01`, `CLI-01` transport, tests | Validated prompt reference, situation fields, contextual time, metadata, language, opaque `InvocationContext`, and pinned identities; only `API-01`'s platform adapter may construct trusted context | Intrinsic request-construction, compiler-compatibility, sealed-context, boundary, and authority tests; no ambient information discovery or transport-owned trust |
...
| `IF-FOCUS-CANDIDATES` | `PLAN-01` | `PLAN-02` | Complete focus propositions after `deriveRequestPropositions(Q, Lambda_A, K)` creates an ephemeral source-locator-bound `RequestPropositionSet` and compatible consolidation joins request support with authorized memory support | Five-field source-receipt, source-locator, duplicate, conflict, authority, provenance, bounded-cardinality, and empty-memory situation-only tests; request propositions never become memory or expectation truth |
...
| `IF-SITUATION` | `SIT-01` | `RET-01`, `SIG-01`, `PLAN-01`, diagnostics | Deterministic numerical `Q` plus exact validated source-byte locators in `X_Q` from prompt/situation/time/metadata and pinned encoders | Same input/snapshot identity gives same bytes and bindings; malformed or mismatched locators fail; no ambient discovery, authority inference, raw-text reread by focus, or focus consolidation |
...
| `IF-COMPILE-API` | `API-01` | `CLI-01`, `OBS-01`, `SEC-01`, `PERF-01`, `SYS-01` | Single local read-only compile call; sole platform-authenticated context construction; intrinsic `CompileRequestError` separated from compiler/configuration compatibility and stage errors; cancellation and complete bytes | Context-forgery, request/error-boundary, byte, capability, retryability, and cancellation tests; compiler/configuration rolls back atomically |
| `IF-CLI` | `CLI-01` | Humans, agents, `SEC-01`, `PERF-01`, `SYS-01` | Terminal request bytes, prompt-origin material, and requested installed-identity transport; stdout framing, stderr diagnostics, typed exit map, and discard-on-failure rule | Golden transport, forgery, construction-versus-compatibility mapping, and broken-pipe tests; no trusted-context construction, trust/configuration resolution, compiler, or management semantics |
...
| CLI and diagnostics | `API-01` is merged; only its platform adapter constructs `InvocationContext`; neither consumer reimplements compiler semantics | Public errors, trusted-context construction, installed-configuration resolution, result bytes, redaction, or authority changes serialize through `API-01`/`SEC-00` |
...
| --- | --- | --- | --- |
| `V1-R01` | `TGT-01`, `CORE-02`, `SIT-01`, `PLAN-01`, `API-01`, `CLI-01` | Intrinsic request versus installed-compatibility errors, source locators, five-field request-proposition receipts, empty-memory situation-only focus, origin, trusted-time, count, metadata, language, and authority-isolation cases | G8 |
...
| `V1-R10` | `SEC-00`, `API-01`, `CLI-01`, `SEC-01`, `RCV-01` | Sole compiler-owned context construction, CLI forgery rejection, prohibited capability, runtime topology, and dependency-graph audits | G8/G10 |
...
| --- | --- | --- | --- | --- |
| `V1-R01` | `PC`, `ARCH` input | `IF-DOMAIN-REQUEST`, `IF-SITUATION`, `IF-FOCUS-CANDIDATES`, `IF-COMPILE-API`, `IF-CLI` | Intrinsic malformed request, valid-but-incompatible request, source-locator/receipt mismatch, empty-memory situation-only focus, language/principal, and unknown-field fixtures | G8 boundary receipt |
...
| `V1-R10` | `PC`, `ARCH` process capability | `IF-SECURITY-ARCH`, `IF-COMPILE-API`, `IF-CLI` | CLI context forgery, forbidden management/network/downstream-model construction, sole trusted-adapter ownership, and topology audit | G8/G10 |
...
| `OD-03` / Open | Stable compiler constructors, request/result types, intrinsic `CompileRequestError` versus compiler/configuration compatibility and stage errors, retryability, cancellation/resource behavior; CLI transport/usage/exit mapping remains adapter-owned | `API-01` with `BND-01`, `SEC-00`, and `TGT-01`; `CLI-01` only maps the accepted typed API | Accepted API ADR plus construction/compatibility boundary fixtures; blocks `API-01`, then `CLI-01` |
| `OD-04` / Open | OS identity, installation principal, caller capability, origin attestation, untrusted metadata treatment, and the `API-01` platform adapter that solely constructs `InvocationContext` | `SEC-00` before `CORE-02` and `MEM-01`; `API-01` implements, `CLI-01` only transports | Security ADR, forgery misuse-case, and sole-owner receipt; blocks domain/security consumers |
...
| `FND-028` / P1 | Library API and CLI transport were conflated / API, delivery | Accepted; `API-01` solely owns platform-authenticated `InvocationContext` construction and installed compatibility, while `CLI-01` transports only request/origin/identity data and maps typed errors | Resolved in DAG, ownership, tests, trace and open decisions |
| `FND-029` / P1 | No package owned prompt/situation-to-numerical-state construction or request-supported focus provenance / architecture, data | Accepted; `SIT-01` owns `Q` and validated source locators, while `PLAN-01` owns ephemeral `RequestPropositionSet` derivation and five-field receipt binding | Resolved in contract and DAG; encoder composition remains evidence-gated |
