...
| Architecture | Request authority was attached to `Compiler::open` | Installation-stable trust is opened once; request-specific `InvocationContext` is passed to every `compile` call |
...
```mermaid
flowchart TD
    IN["Authenticated prompt, situation, contextual time, metadata"] --> Q["Validated numerical situation"]
...
| ID | Concise title, objective, scope, and non-scope | Depends on | Acceptance and exit evidence | Contributes to |
| --- | --- | --- | --- | --- |
| `CORE-01` | **Implement canonical primitives.** Add validated identifiers, finite unit values, presence states, schema and configuration identities, provenance roots, dependency groups, authority labels, and canonical ordering; no storage or runtime model | Passing G1 receipt from `EVD-02`, `BND-01` | Public-contract tests, arbitrary-order properties, and dependency review | G3 entrance |
...
| ID | Concise title, objective, scope, and non-scope | Depends on | Acceptance and exit evidence | Contributes to |
| --- | --- | --- | --- | --- |
| `EXP-01` | **Implement predictive domain contracts.** Add transition, condition, horizon, outcome, status, alternative-family, and query records; no prediction algorithm | `CORE-02`, `EVAL-02` | Constructor, equivalence, contradiction, exclusivity, and lineage tests | G3 and G7 |
| `EXP-02` | **Implement the deterministic expectation baseline.** Add hard eligibility, facet compatibility, dependency budgets, grouping, medoids, relative support, diagnostics, finite alternatives, and abstention; no learned predictor or probability | `EXP-01` | Hand calculations, F13–F16 properties, and exhaustive small-case oracle | G3 and G7 |
| `EXP-03` | **Evaluate expectation semantics.** Add a versioned evaluator and curated corpus for alternatives, dependence, unknown support, abstention, later observations, and numerical boundaries; no runtime calibration | `EXP-02`, `EVAL-02` | Reproducible reports, counterexample coverage, and two intentionally different kernel configurations | G3 and G7 |
| `PLAN-01` | **Construct focus candidates.** Own the complete `FocusCandidateSet` and all request-local compatible-proposition consolidation over a frozen activated input; no final budget selection or shared-set construction | `CORE-02`, `EVAL-02` | Duplicate, conflict, provenance, and bounded-candidate tests | G4 entrance and G7 |
| `PLAN-02` | **Select the combined plan.** Add `FocusExpectationPlan`, mandatory closure, exhaustive reference selector, cost contract, plan shapes, controls, and canonical serialization; no lexical generation or optimized approximation | Passing G3 receipt from `EXP-03`, `PLAN-01` | Exact small-fixture oracle, budget boundaries, alternatives, authority, control-only, and no-action tests | G4 entrance and G7 |
...
| `MEM-04` | **Implement recovery and migration.** Add backup, restore, forward migration, interrupted-migration recovery, integrity verification, and supported rollback; no semantic correction | `MEM-03`, `TGT-01` | Versioned fixtures and backup, restore, migration, and rollback rehearsal without data loss | G5 and G10 readiness |
...
| `SIT-01` | **Construct the numerical situation.** Deterministically transform the validated prompt, situation statements, explicit metadata, contextual time, and pinned encoder identities into the typed request-local situation; do not discover ambient state or infer authorization | `CORE-02`, `ENC-01`, `TGT-01` | Field-ablation, missing/unknown, language, time, order, schema, encoder-incompatibility, and no-discovery tests | G6 and G7 entrance |
...
| `API-01` | **Implement the callable compiler API.** Add `nemosyne-compiler` open and compile boundaries, trusted invocation context, authenticated installed-configuration resolution, cancellation, exact in-memory framing, stable typed errors, and one complete result or error; no terminal transport or memory management | `COMP-01`, `EXP-03`, `PLAN-02`, `REN-06`, `MEM-03`, `SEC-00`, `TGT-01` | One-call golden cases, prompt-byte properties, cancellation, forbidden-capability, and typed-error tests | G8 entrance |
| `CLI-01` | **Implement the CLI adapter.** Add argument parsing, input and installed-configuration-identity transport, API invocation, buffered stdout attempt, diagnostic separation, stable exit mapping, and partial-transport failure behavior; perform no trust resolution and duplicate no compiler logic | `API-01` | CLI golden cases, arbitrary-byte ingress, exit-code, broken-pipe, partial-write, and discard-rule tests | G8 |
...
| `CORE-01` | Canonical terminology → validated dependency-light primitives; no storage or model runtime | `nemosyne-core` | Implementation / P0 / M-high / semantic-core / Stack 1/2 / M2 | Unsafe forbidden, no private data or persistent migration; revert before downstream API stabilizes |
...
| `EXP-01` | Shared-set types → transition, query, and alternative domain API; no support calculation | `nemosyne-core`, expectation spec | Implementation / P0 / L-medium / predictive-semantics / Stack 1/2 / M2 | Authority, provenance, and exact sidecars required; no physical DB migration yet |
| `EXP-02` | Valid transitions → deterministic per-frame expectation sets; no learned predictor or probability | `nemosyne-core` expectation module | Implementation / P0 / L-medium / predictive-semantics / Stack 2/2 / M2 | Bounded input and fail-closed dependency handling; exhaustive oracle permits rollback |
| `EXP-03` | `EXP-02`, corpus registry → expectation reports and curated cases; no runtime calibration | evaluation and corpus crates | Evaluation / P0 / M-high / evidence / Normal / M2 | Synthetic or deidentified fixtures; content-version rollback and immutable failures |
| `PLAN-01` | Shared activated set → complete `FocusCandidateSet`; no shared-set construction or final budget selection | `nemosyne-core` focus module | Implementation / P0 / L-medium / planning / Normal / M2 | Preserve exclusions and authority, no persistence; revert request-local module independently |
| `PLAN-02` | Focus and expectation sets → canonical bounded plan; no lexical generation or approximation | `nemosyne-core` planning module | Implementation / P0 / L-medium / planning / Normal / M2 | No action or answer fields; exhaustive oracle retained as rollback and reference |
| `REN-01` | Canonical plan and target envelope → deterministic attention text and exact substitution; no neural model | new renderer crate and fixtures | Implementation / P1 / L-medium / renderer / Normal / M3 | Injection-safe slots, no network or private logging; candidate is independently removable |
| `REN-02` | Plan and target contracts → exhaustive field mapping, renderer dataset, and corruptions; no training adoption | offline data tooling | Implementation / P1 / L-medium / renderer-data / Normal / M3 | Dataset licensing, provenance, privacy, split, and deletion; mapping rollback by content ID |
| `REN-03` | Frozen renderer data → optional bridge/model candidate artifacts; no release selection | offline training scripts/artifacts | ResearchSpike / P1 / L-low / renderer-ML / Normal / M3 | Artifact sandbox, no private memory, supply-chain inventory; delete or waive cohort without runtime migration |
| `VAL-01` | Renderer schema and corruption data → frozen independent validator identity; no candidate qualification | validator crate and corruption harness | Implementation / P1 / L-medium / renderer-validation / Normal / M3 | Independent implementation/data review, private-content minimization, no migration; stop and delete candidate if false-accept gate fails |
| `REN-04` | Frozen deterministic renderer and validator → qualification receipt; no generative selection or threshold change | offline qualification harness | Qualification / P1 / M-medium / renderer-validation / Normal / M3 | Independent evaluator and signed identities; retained stop receipt and no deployment on failure |
| `REN-05` | Qualified validator plus model candidates → optional generative comparison receipt; no deployment mutation | offline qualification harness and artifacts | Qualification / P1 / L-low / renderer-ML / Normal / M3 | Language/resource/subgroup failures retained; rejected candidates are removable |
| `REN-06` | Qualified deterministic baseline and optional passing model → one authenticated deployment manifest; no training | renderer selection ADR and artifact registry | Decision / P1 / M-medium / renderer / Normal / M3 | Select smallest passing artifact; rollback atomically to prior renderer-plus-validator identity |
| `MEM-01` | Logical memory contract and threat model → physical-storage decision; no production store | prototype area, benchmark docs, ADR | ResearchSpike / P1 / L-low / persistence / Normal / M4 | Encryption, permissions, deletion, recovery threat review; prototypes disposable |
| `MEM-02` | Accepted storage choice and domain records → read-only revision API; no management writes in compile | new memory crate, schema, migrations | Implementation / P1 / L-low / persistence / Normal / M4 | Authorization-before-search, corruption isolation, encrypted/private storage posture; forward and restore fixtures provide rollback |
| `MEM-03` | `MEM-02` → minimum privileged initialization, import, and publication adapter; no correction, deletion, migration, backup, or compile write | dedicated management crate or binary, never a compiler dependency | Implementation / P1 / L-low / persistence-management / Normal / M4 | Dedicated principal and capability, crash atomicity, authenticated import; discard unpublished revision on rollback |
| `MEM-04` | Published revisions → backup, restore, migration, and rollback tools; no semantic correction | privileged recovery modules and fixtures | Implementation / P1 / L-low / persistence-recovery / Normal / M4 | Backup confidentiality, interrupted migration, previous schema retained until verified restore |
| `MEM-05` | Published revisions → authorized export/deletion and derived-state invalidation; no consolidation | privileged privacy modules and receipts | Implementation / P1 / M-low / privacy / Normal / M4 | User authority, complete index invalidation, backup-retention policy; restore only through explicit authorized path |
| `ENC-01` | Exact-plane revision → rebuildable typed numerical facets; no claim of invertible embeddings | encoder adapter and artifact registry | Implementation / P1 / L-low / representation / Normal / M4 | Artifact authenticity, exact-value minimization; rebuild permits rollback |
| `SIT-01` | Validated request fields and pinned encoders → one numerical situation; no discovery or authority inference | compiler/core situation module | Implementation / P1 / L-medium / situation / Normal / M4 | Untrusted metadata remains data, no ambient reads, no migration; reject incompatible schema and roll back encoder/config identity |
| `RET-01` | Authorized revision, facets, and situation → bounded candidates/diagnostics; no post-search authorization | memory/retrieval modules | Implementation / P1 / L-medium / retrieval / Normal / M4 | Noninterference, candidate DoS, stale index; exact scan is reference fallback |
| `SIG-01` | Situation and candidates → typed normalized signals/gates; no learned hidden defaults | compiler/core signal module | Implementation / P1 / L-medium / activation / Normal / M4 | Finite and presence validation, no private logs or migration; versioned configuration rollback |
| `ACT-00` | Frozen signals and evaluation interfaces → content-identified activation parameter candidate; no runtime adoption | offline evaluation/corpus crates and manifests | Evaluation / P1 / M-medium / activation-evidence / Normal / M4 | Synthetic or governed evidence only, no migration; retain all failed candidates and delete no receipt |
| `ACT-01` | Frozen parameter evidence → adopted activation kernel/configuration; no calibration or goal/action selection | core activation and ADR/spec | Implementation / P1 / M-medium / activation / Normal / M4 | Explainability and deterministic failure; retain prior kernel as comparison, not parallel truth |
| `COMP-01` | Adopted activation and eligible sources → complete shared activated set; no focus consolidation or persistent consolidation | compiler/core shared-set builder | Implementation / P1 / L-medium / activation / Normal / M4 | Provenance, authority, and conflict preservation; request-local state is disposable |
...
| `API-01` | Qualified components → in-memory callable compile API; no terminal transport, memory management, or downstream model call | new compiler crate | Integration / P1 / L-medium / integration / Normal / M5 | Trusted context, cancellation, exact bytes, typed errors; rollback compiler/config atomically |
| `CLI-01` | Callable API → terminal ingress and delivery adapter; no compiler logic or management command | new CLI crate | Integration / P1 / M-high / integration / Normal / M5 | Untrusted arguments bounded, diagnostics separated, no migration; rollback adapter independently when API compatible |
...
| `CORE-01` | Core API owner / prevents invalid primitive state | `V1-R06`, `V1-R11`, `V1-R14` through `V1-R21`, `V1-R25`; `MEM`, `EXP`, `PLAN` | `D14`, `D15` | Rustdoc and value/ID APIs; finite/canonical proofs; no persistence migration | Constructor, ordering, finite-domain, and property tests |
...
| `EXP-01` | Predictive-domain owner / makes transitions constructible and rejectable | `V1-R14` through `V1-R23`, `V1-R25`; `EXP` | `D14` | Rustdoc and predictive records; equivalence/compatibility definitions; physical migration deferred to `MEM-02` | Constructor, exclusivity, status, contradiction, and lineage tests |
| `EXP-02` | Expectation-kernel owner / supplies deterministic predictive baseline | `V1-R16` through `V1-R22`, `V1-R25`; `EXP`, `PROOF` | `D14` plus New ADR for adopted thresholds | Expectation API and canonical formulas; no persistent migration | Hand calculations, exhaustive oracle, metamorphic and F13–F16 tests |
| `EXP-03` | Expectation-evaluation owner / measures alternatives and abstention | `V1-R16` through `V1-R22`; `EXP`, `PROOF`, `DEL` | none | Offline evaluator/corpus; no runtime mathematics or migration | Reconstructible reports and adversarial coverage |
| `PLAN-01` | Focus owner / produces complete evidence-bound focus candidates | `V1-R06` through `V1-R08`, `V1-R15`, `V1-R25`; `MEM`, `PLAN` | `D14`, `D15` | Focus API and consolidation definitions; no persistence migration | Duplicate, conflict, authority, and bounded-candidate tests |
| `PLAN-02` | Plan owner / deterministically joins focus and expectation | `V1-R06` through `V1-R08`, `V1-R15` through `V1-R25`; `PLAN`, `PROOF` | `D15`; New ADR only for approximation | Plan API, canonical exhaustive finite-subset lexicographic objective/reference, cost formulas and serialization; an optimized implementation must prove exact equivalence; no migration | Exhaustive small-case equivalence oracle, budget, shape, control, and authority tests |
...
| `MEM-04` | Recovery owner / preserves user memory across versions and failure | `V1-R04`, `V1-R09`, `V1-R13`, `V1-R14`; `ARCH`, `MEM`, `DEL` | New ADR for compatibility window | Backup/restore/migration APIs and schemas; no semantic formula | Interrupted migration and backup/restore/rollback rehearsal |
...
| `SIT-01` | Situation owner / creates one explicit request-local numerical state | `V1-R01`, `V1-R10`, `V1-R11`, `V1-R25`; `ARCH`, `MEM`, `PROOF` | New ADR only if encoder composition is selected | Situation API and deterministic transform; no persistent migration | Missing/unknown, ablation, order, time, language, and no-discovery tests |
...
| `API-01` | Compiler API owner / exposes the single reusable local call | `V1-R01` through `V1-R10`, `V1-R12`, `V1-R13`, `V1-R15` through `V1-R25`; `PC`, `ARCH`, `PROOF` | New ADR for stabilized API/error compatibility | Public Rust API and Rustdoc; composition only, no new mathematics or persistent migration | Boundary, byte, cancellation, capability, and typed-error tests |
| `CLI-01` | CLI adapter owner / makes the API callable without duplicating semantics | `V1-R01` through `V1-R03`, `V1-R08` through `V1-R10`, `V1-R25`; `PC`, `ARCH` | New ADR only for stable CLI compatibility | CLI contract and exit map; no math or storage migration | Golden input/output, broken transport, exit and discard tests |
...
| `IF-CORE-PRIMITIVES` | `CORE-01` | All semantic and numerical domain packages | Finite values, stable IDs, canonical maps/sets, digests, exact sidecars, and errors | Property tests and forbidden invalid state; breaking Rust API uses a major version |
| `IF-DOMAIN-REQUEST` | `CORE-02` | `SIT-01`, memory authorization, `API-01`, `CLI-01`, tests | Validated prompt reference, situation fields, contextual time, metadata, language, `InvocationContext`, and pinned identities | Boundary fixtures and authority tests; no ambient information discovery |
...
| `IF-TRANSITION-DOMAIN` | `EXP-01` | `EXP-02`, `MEM-02`, `ENC-01`, `EXP-03` | Typed before/action/after transition, status, uncertainty, frame, alternative family, dependency, source, and exact sidecars | Constructor, exclusivity, contradiction, and lineage tests; schema change requires stored-record migration |
| `IF-EXPECTATION-SETS` | `EXP-02` | `EXP-03`, `PLAN-02`, `COMP-01`, learned comparison | Finite per-frame hypotheses, evidence shares, unknown/omitted support, abstention, controls, and errors | Exhaustive reference and metamorphic tests; never exposed as probability or fact |
| `IF-EXPECTATION-REPORT` | `EXP-03` | `PLAN-02`, G3 audit, `E2E-00/01` | Reconstructible component outcomes and adversarial-case coverage | Recompute from payload plus manifest; corrections create a new receipt |
| `IF-FOCUS-CANDIDATES` | `PLAN-01` | `PLAN-02` | Complete request-local evidence-bound focus propositions after compatible consolidation | Duplicate, conflict, authority, provenance, and bounded-cardinality tests; no persistent write |
| `IF-PLAN` | `PLAN-02` | `REN-01..06`, `VAL-01`, `API-01` | Canonical `FocusExpectationPlan`, valid shapes, controls, exact slots, cost, and exhaustive lexicographic selection reference | Exhaustive equivalence, closure, budget, and order tests; approximation requires a new decision |
...
| `IF-RECOVERY` | `MEM-04` | `RCV-01`, `REL-01..03` | Backup, restore, migration, integrity, downgrade, and rollback artifacts/receipts | Interrupted-operation rehearsal without loss; retain previous readable format through verification |
...
| `IF-SITUATION` | `SIT-01` | `RET-01`, `SIG-01`, diagnostics | Deterministic numerical `Q` from validated prompt/situation/time/metadata and pinned encoders | Same input/snapshot identity gives same bytes; no ambient discovery or authority inference |
...
| `IF-SHARED-SET` | `COMP-01` | `PLAN-01`, `EXP-02`, `API-01` | Complete eligible activated-memory set with lineage, authority, conflicts, limits, and canonical order | Deterministic reconstruction and projection noninterference; no proposition consolidation |
...
| `IF-COMPILE-API` | `API-01` | `CLI-01`, `OBS-01`, `SEC-01`, `PERF-01`, `SYS-01` | Single local read-only compile call, trusted invocation, cancellation, typed open/compile errors, and complete bytes | Boundary, byte, capability, retryability, and cancellation tests; compiler/configuration rolls back atomically |
| `IF-CLI` | `CLI-01` | Humans, agents, `SEC-01`, `PERF-01`, `SYS-01` | Terminal argument/input adapter, stdout framing, stderr diagnostics, exit map, and discard-on-failure rule | Golden transport and broken-pipe tests; no compiler or management semantics |
...
→ CORE-02
→ {EXP-01/02/03 and G3, PLAN-01, E2E-00 freezes G9}
→ PLAN-02
→ REN-01/02 → VAL-01 → REN-04 → optional REN-03/05 → REN-06 and G4
→ MEM-01/02/03
  ├─ runtime lane: ENC-01 → SIT-01 → RET-01 → SIG-01 → ACT-00
  │  → ACT-01 → COMP-01 → API-01 → CLI-01 and OBS-01
...
This is a dependency sequence, not a calendar-duration claim. `MEM-01/02/03`
may run in parallel with renderer work after their prerequisites pass, but
`API-01` cannot merge until memory, activation, prediction, planning, renderer,
security, and target paths have produced their named receipts. The actual
calendar critical path remains unknown until every package on the selected
path replaces any low-confidence estimate with measured planning evidence.
...
| `CP-SEMANTIC` | `{EVD-02, BND-01} → CORE-01 → EVAL-01 → EVAL-02 → EXP-01 → EXP-02 → EXP-03 → PLAN-02`, joined with `CORE-02 → PLAN-01` | Defines typed predictive and planning truth consumed by every renderer and end-to-end evaluation | Reject invalid domain or algorithm; retain exhaustive references and failed receipts |
...
| `CP-MEMORY` | `{CORE-02, SEC-00, TGT-01} → MEM-01 → MEM-02 → ENC-01 → SIT-01 → RET-01 → SIG-01 → ACT-00 → ACT-01 → COMP-01` | Produces the complete authorized activated set consumed by both semantic branches | Exact-scan and prior-kernel references support rollback; failed authorization or recovery stops |
...
| `CP-INTEGRATION` | `{COMP-01, EXP-03, PLAN-02, REN-06, MEM-03, SEC-00, TGT-01} → API-01 → {CLI-01, OBS-01} → {SEC-01, PERF-01} → SYS-01` | All implementation lanes join at the reusable call before transport and integrated evidence | Roll back the failing adapter/component; do not bypass through CLI logic |
...
| `M2` | Establish deterministic semantic and evaluation cores; entrance is M1 | `CORE-01`, `CORE-02`, `EVAL-01`, `EVAL-02`, `EXP-01`, `EXP-02`, `EXP-03`, `PLAN-01`, `PLAN-02`, `E2E-00` | G3 and planning-entrance evidence, frozen G9 protocol; permits renderer and persistence work | Reference mismatch, semantic leakage, or unclosed plan |
| `M3` | Qualify one renderer-validator pair; entrance is M2 | `REN-01`, `REN-02`, `VAL-01`, `REN-04`, `REN-06`; `REN-03/05` only if authorized | G4 and deployment manifest; permits compiler integration | Deterministic baseline fails or no validator meets false-accept gate |
| `M4` | Establish authorized memory-to-activation path; entrance is M1/M2 as named per package | `MEM-01..05`, `ENC-01`, `SIT-01`, `RET-01`, `SIG-01`, `ACT-00`, `ACT-01`, `COMP-01` | G5 through G7 and recovery/privacy readiness; permits compiler integration | Data loss, authority leak, unrecoverable migration, or missing recall/activation evidence |
...
| `W07` | `EXP-01` ∥ `PLAN-01` ∥ `MEM-01` ∥ `E2E-00` | Predictive domain, focus boundary, persistence decision, and frozen G9 protocol |
...
| `W10` | `PLAN-02` ∥ `MEM-04` ∥ `MEM-05` ∥ `SIT-01` | Closed plan, recovery/privacy readiness, and numerical situation |
| `W11` | `REN-01` ∥ `RET-01` | Deterministic lexical baseline and authorized retrieval |
| `W12` | `REN-02` ∥ `SIG-01` | Renderer corpus and typed signals |
| `W13` | `VAL-01` ∥ `ACT-00` ∥ optional `REN-03` | Frozen validator, activation candidate, and optional model artifacts |
| `W14` | `REN-04` ∥ `ACT-01` | Qualified deterministic renderer and adopted activation |
| `W15` | `COMP-01` ∥ optional `REN-05` | Shared set and optional model disposition |
| `W16` | `REN-06` | Authenticated renderer-validator deployment |
| `W17` | `API-01` | Reusable one-call compiler boundary |
| `W18` | `CLI-01` ∥ `OBS-01` | Transport and authorized diagnostics |
| `W19` | `SEC-01` ∥ `PERF-01` | Independent security and operational receipts |
| `W20` | `SYS-01` | G8 vertical-slice receipt |
| `W21` | `REL-01` | Frozen content-identified candidate |
| `W22` | `RCV-01` | Exact-package verification receipt |
| `W23` | `E2E-01` | Permanent G9 receipt |
| `W24` | `REL-02` | Signed ship authorization or stop receipt |
| `W25` | `REL-03` | Verified shipment or publication rollback receipt closing G10 |
...
- deterministic renderer feasibility after `PLAN-02`;
...
| Renderer and memory/activation | Both consume frozen plan/target or core interfaces and edit separate artifacts | Target limits, exact-slot types, or shared-set semantics serialize before either consumer |
...
| CLI and diagnostics | `API-01` is merged and neither reimplements compiler semantics | Public errors, result bytes, redaction, or authority changes serialize through `API-01`/`SEC-00` |
...
| --- | --- | --- | --- |
| `V1-R01` | `TGT-01`, `CORE-02`, `SIT-01`, `API-01`, `CLI-01` | Valid/invalid ingress, origin, trusted-time, count, metadata, language, and authority-isolation cases | G8 |
| `V1-R02` | `TGT-01`, `API-01`, `CLI-01`, `SYS-01`, `RCV-01`, `REL-03` | Complete-or-error, partial-transport failure injection, identical request/snapshot repeatability, prohibited request-randomness tests, and published-byte verification | G8/G10 |
...
| `V1-R06` | `CORE-01`, `PLAN-02`, `REN-04`, `REN-06` | Provenance, authority, qualification, conflict, semantic-validator evidence | G4/G8 |
...
| `V1-R09` | `TGT-01`, `MEM-01..05`, `SEC-00`, `SEC-01`, `REL-01`, `RCV-01` | Local path, no-network, artifact trust, packaging, recovery, privacy, and permission receipts | G8/G10 |
| `V1-R10` | `SEC-00`, `API-01`, `CLI-01`, `SEC-01`, `RCV-01` | Prohibited capability, runtime topology, and dependency-graph audits | G8/G10 |
...
| `V1-R14` | `EXP-01`, `MEM-02` | Transition schema, status, provenance, version, and persistence fixtures | G3/G5 |
| `V1-R15` | `CORE-01`, `PLAN-02`, `REN-04`, `REN-06` | Type-boundary and semantic-corruption suite | G4 |
| `V1-R16` | `EXP-02`, `PLAN-02` | Empty, single, alternative, tie, finite-limit, and order properties | G3 |
| `V1-R17` | `EXP-02`, `PLAN-02`, `REN-04`, `REN-06` | Complete expectation closure and renderer fidelity | G4 |
| `V1-R18` | `EXP-02` | Duplicate and dependency-budget metamorphic tests | G3 |
| `V1-R19` | `EXP-02`, `PLAN-02`, `REN-04`, `REN-06` | Evidence-share labels and forbidden probability promotion | G4 |
| `V1-R20` | `EXP-02`, `PLAN-02` | Every valid-insufficiency abstention reason and malformed-canonicalization, dependency, lineage, and global-budget error distinction | G3/G7 |
...
| --- | --- | --- | --- | --- |
| `V1-R01` | `PC`, `ARCH` input | `IF-DOMAIN-REQUEST`, `IF-SITUATION`, `IF-COMPILE-API`, `IF-CLI` | Invalid prompt/situation/time/metadata/language/principal and unknown-field fixtures | G8 boundary receipt |
| `V1-R02` | `PC`, `ARCH` one-result determinism | `IF-COMPILE-API`, `IF-CLI`, `IF-RELEASE-CANDIDATE`, `IF-SHIPMENT` | Same input/snapshot byte equality, same typed error, cancellation and partial-delivery failure | G8, exact-package G9, G10 shipment |
...
| `V1-R06` | `PC`, `MEM`, `PLAN`, `REN` grounding | `IF-SHARED-SET`, `IF-FOCUS-CANDIDATES`, `IF-PLAN`, `IF-VALIDATOR` | Missing lineage, unsupported proposition, source/control closure and semantic-corruption cases | G4/G8 |
...
| `V1-R09` | `PC`, `ARCH` local installation | `IF-TARGET-ENVELOPE`, `IF-SECURITY-ARCH`, `IF-RELEASE-CANDIDATE`, `IF-RCV-RECEIPT` | Network denial, artifact authenticity, path/permission, recovery and privacy cases | G8/G10 |
| `V1-R10` | `PC`, `ARCH` process capability | `IF-SECURITY-ARCH`, `IF-COMPILE-API`, `IF-CLI` | Forbidden management/network/downstream-model construction and topology audit | G8/G10 |
...
| `V1-R14` | `PC`, `EXP`, `MEM` transitions | `IF-TRANSITION-DOMAIN`, `IF-MEMORY-REVISION` | Invalid status, missing source/version/time, contradiction and migration fixtures | G3/G5 |
| `V1-R15` | `PC`, `EXP`, `PLAN`, `REN` type boundaries | `IF-EXPECTATION-SETS`, `IF-FOCUS-CANDIDATES`, `IF-PLAN`, `IF-VALIDATOR` | Focus/condition/expectation/action/fact/probability promotion corruption cases | G4 |
| `V1-R16` | `PC`, `EXP` finite hypotheses | `IF-EXPECTATION-SETS`, `IF-PLAN` | Empty, single, alternative, tie, limit and order cases | G3/G7 |
...
| `V1-R18` | `PC`, `EXP` dependence | `IF-TRANSITION-DOMAIN`, `IF-EXPECTATION-SETS`, `IF-SHARED-SET` | Duplicate, correlated-source, dependency-budget and hidden-dependence sensitivity | G3/G7 |
...
| `V1-R20` | `PC`, `EXP`, `PLAN` abstention/error | `IF-EXPECTATION-SETS`, `IF-PLAN`, `IF-COMPILE-API` | Every valid insufficiency reason versus malformed canonicalization/dependency/lineage/budget error | G3/G7/G8 |
...
| `RSK-20` | Compile gains persistent-write, network, management, or downstream-agent capability | Low / Critical / Open | `SEC-00`, `API-01`, `SEC-01`; `V1-R04`, `V1-R10`, `V1-R13`, `V1-R23` | Dependency/capability graph proof and runtime denial; any path blocks G8 |
...
| `OD-03` / Open | Stable compiler constructors, request/result types, `OpenError`/`CompileError` sources, retryability, cancellation/resource behavior; CLI transport/usage/exit mapping remains adapter-owned | `API-01` with `BND-01`, `SEC-00`, and `TGT-01`; `CLI-01` only maps the accepted API | Accepted API ADR plus boundary fixtures; blocks `API-01`, then `CLI-01` |
| `OD-04` / Open | OS identity, installation principal, caller capability, origin attestation, and untrusted metadata treatment | `SEC-00` before `CORE-02` and `MEM-01` | Security ADR and misuse-case receipt; blocks domain/security consumers |
...
| `OD-13` / Open | Source schema window, forward migration, interruption, backup format, restore, downgrade and rollback | `MEM-04` before `REL-01` | Accepted compatibility ADR and lossless rehearsal; blocks candidate freeze |
...
| `OD-19` / Open | Expectation materiality, coverage, effective groups, unknown mass, and `Kmax` | `EXP-02` with `EXP-03` evidence | Frozen configuration decision; blocks G3 |
...
| --- | --- | --- | --- |
| `REV-01` | Product reviewer; no architecture ownership | Planned; reviewer/digest pending; inspect `FND-007`, `FND-016`, `FND-030` | Verify product/G1/release-state contracts and trace links; owner principal architect/release authority |
| `REV-02` | Cognitive-science reviewer; no software-contract ownership | Planned; reviewer/digest pending; inspect `FND-001`, `FND-004`, `FND-015` | Verify alternative/assessment/non-biological boundaries; owner predictive-semantics owner |
| `REV-03` | Formal-mathematics reviewer; no implementation ownership | Planned; reviewer/digest pending; inspect `FND-001..003`, `FND-020..024` | Verify total domains, exhaustive reference and notation registry; owners expectation/planning |
| `REV-04` | Statistics/evaluation reviewer; no product-scope ownership | Planned; reviewer/digest pending; inspect `FND-002`, `FND-017`, `FND-018`, `FND-034` | Verify pre-outcome G1/G9 manifests, root splits and retained failures; owner evidence architect |
| `REV-05` | Rust/API reviewer; no product or CLI ownership | Planned; reviewer/digest pending; inspect `FND-009`, `FND-010`, `FND-028` | Verify domain/compile interfaces and typed errors; owner compiler API |
| `REV-06` | Data-model reviewer; no storage-selection ownership | Planned; reviewer/digest pending; inspect `FND-003`, `FND-004`, `FND-023`, `FND-029` | Verify exact/numerical split, transition/situation and migration interfaces; owners domain/memory/situation |
| `REV-07` | Security/privacy reviewer; no runtime architecture ownership | Planned; reviewer/digest pending; inspect `FND-009`, `FND-010`, `FND-011`, `FND-031` | Verify capability, trust, privacy and interface registries; owner security architect |
| `REV-08` | Adversarial-robustness reviewer; no algorithm ownership | Planned; reviewer/digest pending; inspect `FND-002`, `FND-005`, `FND-008`, `FND-020` | Verify counterexamples, poisoning/noninterference and fail-closed behavior; relevant algorithm owners |
| `REV-09` | ML/rendering reviewer; no renderer-selection ownership | Planned; reviewer/digest pending; inspect `FND-006`, `FND-008`, `FND-014`, `FND-025`, `FND-036` | Verify deterministic baseline, optional cohort, validator independence and typed learned slots; renderer owners |
| `REV-10` | Performance reviewer; no target-envelope ownership | Planned; reviewer/digest pending; inspect `FND-021`, `FND-026`, `FND-032` | Verify bounded domains, complexity estimates and measurement gates; target/performance owners |
| `REV-11` | Operability/release reviewer; no component ownership | Planned; reviewer/digest pending; inspect `FND-011`, `FND-012`, `FND-030`, `FND-037` | Verify failure transport, management/recovery, release state and EOL; operations/release owners |
| `REV-12` | Cross-platform reviewer; no numerical owner | Planned; reviewer/digest pending; inspect `FND-003`, `FND-012`, `FND-024` | Verify canonical order, bytes, path, time, float and artifact identity; core/integration owners |
| `REV-13` | Maintainability reviewer; no canonical-subject ownership | Planned; reviewer/digest pending; inspect `FND-005`, `FND-026..031`, `FND-035` | Verify package, responsibility, interface and conformance registries; principal architect/interface owners |
| `REV-14` | Delivery-program reviewer; no product/math ownership | Planned; reviewer/digest pending; inspect `FND-016..019`, `FND-026`, `FND-030`, `FND-032..035` | Verify DAG, waves, milestones, gates and receipts; program/release owners |
| `REV-15` | Reliability/observability/failure-recovery reviewer; no diagnostics or recovery ownership | Planned; reviewer/digest pending; inspect `FND-004`, `FND-012`, `FND-030`, `FND-037` | Verify typed failures, reconstruction, cancellation, crash/migration, rollback and publication failure; reliability owners |
| `REV-16` | Downstream-agent integration/usability reviewer; no compiler/CLI ownership | Planned; reviewer/digest pending; inspect `FND-007`, `FND-012`, `FND-028` | Verify one-call usability, byte/discard semantics, controls, examples and unsupported behavior; API/CLI/product owners |
| `REV-17` | Documentation-governance/historical-correctness reviewer; no decision authorship | Planned; reviewer/digest pending; inspect `FND-006`, `FND-007`, `FND-024`, `FND-026`, `FND-035` | Verify ownership, supersession, historical immutability, statuses and non-claims; principal architect |
| `REV-18` | Testing/CI/release/supply-chain reviewer; no packaging ownership | Planned; reviewer/digest pending; inspect `FND-017..019`, `FND-030..037` | Verify layered tests, CI gates, manifests, signing/SBOM, candidate identity and publication rollback; release/security owners |
...
| `FND-009` / P1 | Request authority was retained at open / API, security | Accepted; request-specific `InvocationContext` on every compile; `API-01` | Resolved in contract; G8 capability tests remain |
...
| `FND-020` / P1 | Abstention lacked a source-bound proposition / mathematics, API | Accepted; exclusive `FrameAbstentionCandidate`; `EXP-02` | Resolved in contract; G3/G4 thresholds remain |
...
| `FND-023` / P1 | Transition status/uncertainty ownership was incomplete / data, reliability | Accepted; canonical observation schema and distinct uncertainties; `EXP-01` | Resolved in contract; physical schema remains `OD-12` |
...
| `FND-028` / P1 | Library API and CLI transport were conflated / API, delivery | Accepted; split `API-01` and adapter-only `CLI-01`; compiler owner | Resolved in DAG, trace and open decisions |
| `FND-029` / P1 | No package owned prompt/situation-to-numerical-state construction / architecture, data | Accepted; add `SIT-01` and `IF-SITUATION`; situation owner | Resolved in contract; encoder composition remains evidence-gated |
...
| `FND-031` / P1 | Cross-package artifacts had no sole owner/version/rollback / maintainability, security | Accepted; interface/artifact registry and digest chain; interface owners | Resolved in contract; formats remain package work |
...
| `FND-037` / P1 | Compatibility, deprecation, EOL and post-publication checks were incomplete / operability | Accepted; `OD-29`, release lifecycle, `REL-03`; release owner | Resolved in contract; policy selection remains |
...
| G7 | `EXP-01`, `EXP-02`, `EXP-03`, `PLAN-01`, `PLAN-02`, `SIT-01`, `SIG-01`, `ACT-00`, `ACT-01`, `COMP-01` | Situation, signal, parameter freeze, activation, shared-set construction, focus, expectation, and combined-planning component evidence |
