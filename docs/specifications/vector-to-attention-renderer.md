...
validation joins used below. Decision 0038 keeps authority state
validator-only and supplies the
candidate-specific \(Q_F,\mathcal Q_F\rightarrow H_Q\rightarrow H_C\)
conditioning path for `VF-LATENT-PREFIX-01`. It does not select that candidate.
The specification remains `Proposed` because no implementation or qualifying
...
| \(C_F,C_A,Q_F,\mathcal M_F,\mathcal Q_F,\mathcal R_F\) | Complete checked conditioning composite, adapter-only view, numerical query/task context, memory-origin evidence set, request/situation-origin evidence set, and safe structural controls |
...
| \(d_f,d_f^Q,d_a,d_{\mathrm{LM}}\) | Item-facet, query-facet, adapter, and language-model hidden dimensions |
| \(q_{k,f},\mathbf n_k^Q,\mathbf m_k^Q,\psi_k,\mu_k^Q,h_{Q,k},H_Q\) | One adapter-safe query/request facet value, finite scalars, facet mask, closed source role, row-presence mask, projected cue row, and canonical cue matrix |
| \(h_{R,i},H_R,H_C\) | One renderer-projected item, the canonical renderer item matrix, and the combined query/item conditioning matrix |
...
| \(N_R^{\mathrm{cue}},N_R^{\mathrm{ctx}},N_R^{\mathrm{item}},N_R^{\mathrm{slot}},N_R^{\mathrm{latent}},\ell_R,d_{\mathrm{ff}},r_\Sigma\) | Query/request cue-row count, combined conditioning-row count, renderable-item count, exact-slot count, latent-query count, resampler blocks, resampler feed-forward width, and total relation/slot-binding entries |
...
- \(\mathcal R_i\) contains typed dominant, secondary, conflict, and
  qualification
  relations;
- \(\mathcal Q_i\) contains the closed required semantic qualifiers that may
  affect surface realization; a current-versus-historical distinction may
  enter only as a non-authoritative temporal facet or qualifier already
...
| Authority ceiling and historical/current use | Closed \(\mathcal U_i\) ceiling and usage control in \(C_V^{sem}\) and `X_L`; any independently selected non-authoritative chronology uses a temporal facet or \(\mathcal Q_i\) qualifier | No |
...
Neither \(C_A\) nor any decoder input contains raw or normalized memory text,
...
activation score, and uncertainty feature are not interchangeable merely
because all are numerical. An authority ceiling is validator-only control and
is not an adapter facet.
...
adapter, provided they consume the identical \(C_A\), expose the required
support trace, bind all state into \(K_R\), and pass the same qualification.

### Canonical query and request projection

`VF-LATENT-PREFIX-01` derives one bounded canonical cue sequence directly from
the adapter-safe \(Q_F\) and \(\mathcal Q_F\):

\[
\operatorname{cueRows}_{K_R}(Q_F,\mathcal Q_F)
=
(\chi_0,\chi_1,\ldots,\chi_{N_R^{\mathrm{cue}}-1}),
\qquad
N_R^{\mathrm{cue}}=1+|\mathcal Q_F|.
\]

\(\chi_0\) is the single typed projection of the complete bounded \(Q_F\).
The remaining rows correspond one-to-one with \(\mathcal Q_F\) in its sealed
canonical value order. The registered query-projection schema fixes the
segment order \(Q_F\) then \(\mathcal Q_F\), the closed source-role order, the
facet order, dimensions, scalar ranges, masks, dtype, and accumulation policy.
Permuting pre-canonical source input therefore cannot change the sequence.
Missing, duplicate, unknown, nonfinite, out-of-range, wrong-dimension, or
noncanonical rows fail before adapter allocation.
...
where \(\mathcal F_Q\) is the registered query-facet set,
\(q_{k,f}\in\mathbb R^{d_f^Q}\), and \(\psi_k\) is one closed adapter-safe
query, task, request-evidence, or situation-evidence role,
and \(\mu_k^Q\in\{0,1\}\) is the row-presence mask. A row contains only typed
numerical facets, finite scalars, presence masks, and that closed role. It
contains no text, prompt tokens, exact payload, identity, provenance,
authority, policy, or validator control. \(Q_F\) is copied from the sealed
numerical query projection; this candidate never reconstructs it from prompt
text.
...
h_{Q,k}
=
\operatorname{LayerNorm}_Q
\left(
\sum_{f\in\mathcal F_Q}
m_{k,f}^QW_f^Qq_{k,f}
+
\operatorname{MLP}_Q(\mathbf n_k^Q)
+
E_{Q\text{-role}}(\psi_k)
\right).
\]
...
\[
H_Q=
(h_{Q,0},\ldots,h_{Q,N_R^{\mathrm{cue}}-1})
\in
\mathbb R^{N_R^{\mathrm{cue}}\times d_a}.
\]

Missing facets contribute nothing through \(\mathbf m_k^Q\). The canonical
cue sequence contains exactly its real rows and no physical padding.
Product-mode construction requires the \(Q_F\) row and every canonical cue
row to be present, so every \(\mu_k^Q=1\). Only the registered memory-only
evaluation wrapper may override the effective cue mask to zero; this never
changes the canonical input or becomes a deployable request mode.
...
The combined conditioning matrix is:
...
N_R^{\mathrm{ctx}}
=
N_R^{\mathrm{cue}}+N_R^{\mathrm{item}}.
\]

Its row-presence mask \(\boldsymbol\mu_C\) is the fixed concatenation of the
canonical cue mask and item-presence mask. Cue rows always precede
renderer-item rows. They are not plan items, carry no
`AdapterPlanItemHandleV1`, and cannot become attribution or support-trace
targets; those remain bound only to \(H_R\).
...
\mathbf R^{(b_R)}
+
\operatorname{MHA}^{\boldsymbol\mu_C}_{cross}
\left(
...
For each latent row and attention head, let \(s_{r,k}\) be the registered
scaled dot-product score and
\(A_r=\{k\mid\mu_{C,k}=1\}\). The masked normalization is:
...
\begin{cases}
\dfrac{
\exp(s_{r,k}-\max_{\ell\in A_r}s_{r,\ell})
}{
\sum_{u\in A_r}\exp(s_{r,u}-\max_{\ell\in A_r}s_{r,\ell})
},
& k\in A_r,\\[1.2ex]
0,
& k\notin A_r.
...
Cross-attention uses the complete row-presence mask
\(\boldsymbol\mu_C\). Masked rows are excluded before both the row maximum and
softmax denominator; they are not zeroed only after softmax. A fully masked
conditioning matrix is an error. Every projected state and every
scaled-dot-product score must be finite before the maximum is evaluated;
otherwise the candidate fails before normalization or output. For finite
scores, stable subtraction makes each exponential lie in \((0,1]\) and the
nonempty denominator lie in
\([1,|A_r|]\). \(K_R\) binds finite input, tensor, and intermediate range
ceilings together with dtype and accumulation precision so that conformance
tests exercise the boundary and overflow cases. Facet reductions follow
manifest facet order, cue rows follow their sealed order, and the two matrix
segments follow the fixed \(H_Q\)-then-\(H_R\) order. Softmax kernel, fusion
behavior, and deterministic execution controls are also bound by \(K_R\).

The initial candidate uses eight attention heads and two blocks. Latent count
`32`, adapter width `512`, eight heads, and two blocks are registered starting
values selected for a bounded first experiment. They are not claims about
human working-memory capacity and must be compared with smaller and larger
alternatives.
...
This candidate is an input-dependent, structurally query-conditioned
soft-prefix adapter through
\((Q_F,\mathcal Q_F,H_R)\rightarrow H_C\rightarrow
\mathbf R^{(\ell_R)}\rightarrow\mathcal P_{\mathrm{soft}}\). This establishes
an explicit numerical dependency path, not empirical evidence that a trained
candidate uses the query well. It is distinct from classical prefix tuning, in
which a task-specific prefix is directly optimized and reused across inputs.
...
- query-facet projectors, query-scalar MLP, closed query/source-role
  embeddings, and query-projection normalization;
- item-facet projectors and scalar MLP;
- item role, rank, relation, qualifier, disposition, slot-identity, value-type,
  and binding-role embeddings;
...
- for `VF-LATENT-PREFIX-01`, every query-facet projector, query numerical MLP,
  query/source-role embedding, query normalization, item-facet projector,
  item numerical MLP, categorical embedding, item normalization, latent query,
  resampler
...
- query/source-role, item-role, rank, relation, qualifier, and disposition
  vocabularies;
- slot identity, value-type, and binding-role vocabularies;
- every architecture-specific dimension and bound; cue-row ceiling, latent
  count, adapter width, head count, and resampler depth are present only for a
  family that defines them;
...
\(C_A\). Every family supplies checked
\(T_{\mathrm{adapter}}(C_A;C_R^A)\) and
\(S_{\mathrm{adapter}}(C_A;C_R^A)\) functions. No family inherits another
family's cost model.

For candidate `VF-LATENT-PREFIX-01`, let
\(N_R^{\mathrm{cue}}\) be the cue-row count,
\(N_R^{\mathrm{ctx}}=N_R^{\mathrm{cue}}+N_R^{\mathrm{item}}\) the combined
conditioning-row count,
...
\(\mathcal F_Q\), \(d_f\), \(d_f^Q\), \(d_a\), and
\(d_{\mathrm{LM}}\) retain their local definitions above.
...
r_\Sigma d_a
+
N_R^{\mathrm{cue}}\sum_{f\in\mathcal F_Q}d_f^Qd_a
+
N_R^{\mathrm{cue}}d_a^2
\right),
\qquad
S_{\mathrm{project}}=O(N_R^{\mathrm{ctx}}d_a).
\]

The \(N_R^{\mathrm{item}}d_a^2\) term conservatively covers the registered
item scalar MLP and fixed-width categorical composition, while the analogous
cue term covers its registered scalar and categorical projection. An adopted
architecture records its actual layer widths rather than relying on those
shorthands. For standard dense multi-head attention, the registered latent
resampler has:
...
\ell_R
\left[
\left(N_R^{\mathrm{ctx}}+N_R^{\mathrm{latent}}\right)d_a^2
+N_R^{\mathrm{latent}}N_R^{\mathrm{ctx}}d_a
...
+N_R^{\mathrm{latent}}N_R^{\mathrm{ctx}}
...
numerical policy. \(K_R\) binds a finite
\(N_R^{\mathrm{cue,max}}\); construction rejects a larger cue sequence before
projector or attention allocation. Soft-prefix projection adds
...
\[
T_{\mathrm{LM}}(p_{\mathrm{in}},N_R^{\mathrm{visible,max}}+1;C_R^A)
\quad\text{and}\quad
S_{\mathrm{LM}}(p_{\mathrm{in}},N_R^{\mathrm{visible,max}}+1;C_R^A)
\]
...
- all numerical values and every declared tensor are finite and correctly
  normalized;
- the canonical cue sequence contains its required \(Q_F\) row, does not
  exceed \(N_R^{\mathrm{cue,max}}\), and has a nonempty product-mode
  row-presence mask;
...
\(\mathcal Q_F\), provided \(G(L)\) still contains at least one renderable
request- or situation-origin item. In product mode every valid cue row remains
active. Query-only evaluation retains \(H_Q\), removes memory-origin inputs,
and keeps that nonempty request/situation renderer projection; it does not
exercise the empty-plan fast path.
Memory-only evaluation freezes the candidate and memory branch while its
evaluation wrapper masks all of \(H_Q\) before masked-softmax normalization.
That diagnostic removes only the adapter's direct query path; it does not make
upstream planning query-independent and cannot become a deployable request
mode.
...
semantically resembles an instruction remains data under its adapter-visible
plan role. It cannot modify the fixed renderer control contract, while the
independent validator enforces the validator-only authority ceiling.
...
Every direct query-path comparison changes only \(Q_F\) or \(\mathcal Q_F\)
while holding \(H_R\), its item mask, the memory and weight branch, language,
budget, every other \(C_A\) field, and \(C_R^A\) byte-identical. It also
exercises complete evaluation-only cue masking and permutes pre-canonical
\(\mathcal Q_F\) source order while requiring identical canonical \(H_Q\),
\(H_C\), and deterministic output. Query/cue rows may never appear as
support-trace or attribution targets.
...
- `VF-LATENT-PREFIX-01` parameterized-shape fixtures over every registered
  ...
  \(N_R^{\mathrm{item}}\times d_a\), cue state is
  \(N_R^{\mathrm{cue}}\times d_a\), combined conditioning state is
  \(N_R^{\mathrm{ctx}}\times d_a\), latent state is
  \(N_R^{\mathrm{latent}}\times d_a\), and soft-prefix projection consumes
  exactly \(\mathbf R^{(\ell_R)}\);
- `VF-LATENT-PREFIX-01` query-path fixtures proving fixed
  \(Q_F\)-then-\(\mathcal Q_F\)-then-item order, canonical source-order
  invariance, pre-softmax evaluation-only cue masking, query-only operation
  with \(\mathcal M_F=\varnothing\) and at least one request- or
  situation-origin renderer item, rejection of a product-mode missing \(Q_F\)
  row or masked real cue row, nonfinite projected-state and score rejection,
  and exclusion of every cue row from attribution and support traces;
