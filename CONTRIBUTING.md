...
Every pull request must declare its documentation impact and explain that declaration. Internally authored Dependabot GitHub Actions updates are the only exception. They still run the structural documentation checks. Every change to production Rust source requires an updated specification. A significant design choice requires a decision record. Tests must verify normative claims where practical.
